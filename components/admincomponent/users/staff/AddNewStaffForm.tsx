...
import {useAddStaffMutation} from "@/lib/features/admin/user-management/staff/staff";
...
export default function AddNewStaffForm() {
    ...
    const [addStaff, { isLoading : isStfLoading, isSuccess : isStfSucess, error: isStfError }] = useAddStaffMutation();

    const [createSingleFile] = useCreateSingleFileMutation();

    const [birthDate, setBirthDate] = React.useState<Date>()

    const handleAddStaff = async (values : any , { setSubmitting ,resetForm } : any) => {

        if (!cvFile || !idCardFile) {
            console.error('CV file or ID card file is missing.');
            return;
        }
        ...
        try {
            ...
            if (values.authorityNames.length > 0) {
                ...
                try {
                    ...
                    setSubmitting(false);// Debugging the error
                }
            }
                ...
                <TabsList className="bg-transparent w-full h-[50px]  rounded-none  ">
                    <div className="bg-transparent w-full flex justify-between items-center gap-4">
                        ...
                        <div className=" flex items-center gap-6">
                            ...
                            <p className="text-lms-primary text-lg ">Set Permissions</p>
                        </div>
                        ...
                        <TabsContent value="permission">
                            <div className=" mx-10 ">
                                <RequiredFieldLabelComponent labelText="Customize Permission"
                                                             labelClassName={` block text-lg font-medium text-gray-900 dark:text-white  mb-6`}/>
                                <div className="grid grid-cols-4 gap-4 justify-center items-start">
                                    {authorityOptions.map(authority => (
                                        <div key={authority} className="flex items-center mb-2">
                                            <Field
                                                ...
                                                onChange={(event: any) => handleCheckboxChange(event, authority)}
                            ...
                            <div className="flex items-center justify-end w-full pr-10 mt-6"
                                 >
                                <Button type="submit"
                                        className="bg-lms-primary text-lms-white-80 hover:bg-lms-primary/90" disabled={isStfLoading}
                                >{isStfLoading ? "Adding..." : "Add"}</Button>
                            </div>
